...
Every time a Claude Code session starts, the project's `CLAUDE.md`, `CLAUDE.local.md` and `.claude/rules/*` are loaded as system instructions, from the launch directory and every directory above it — and so are the user-global ones under `~/.claude/`. They shape Claude's behavior for the entire session. A malicious commit, a compromised dependency that drops one in, an agent that edits your global config, or even an accidental edit can re-program Claude silently.
...
- **SessionStart hook** computes SHA-256 of every instruction file Claude Code loads at launch (see [Scope](#scope-and-threat-model)), compares to hashes stored in `~/.claude/affirm-hashes.json`, and emits a banner on both channels: `systemMessage` for the terminal and `additionalContext` for the model, so a session knows when it is running under unaffirmed instructions. The model's copy swaps the call to action for a guard: affirming is the user's attestation, not something to do on their behalf. New/changed files carry their last-modified age and git info inline so you can judge a change at a glance. The banner is the only thing injected — instruction content never is. The hook also dedupes re-fires per `session_id` (markers at `~/.claude/state/affirm-firstfire/`, pruned after 7 days) so Claude Code's multi-fire lifecycle (startup, resume, /clear, /compact) doesn't surface the banner ten times in one session. A `/compact` fire is the exception: it re-sends the model's copy, because a compaction summary drops the banner from the model's context (measured with a stand-in hook: lost 2 of 2 times, restored when the compact fire went through), while the terminal still has it in scrollback.
...
ℹ 1 @import beyond depth 4 not tracked: docs/c.md → docs/d.md
```
...
`← @from <file>` marks a file pulled in by another's `@import`; `(global)` marks one loaded from `~/.claude/` in every project; `(ancestor)` marks one loaded from a directory above the launch directory; `(out-of-tree)` marks an import or linked rule that lives outside the project. New/changed files get a second line with their modified age and git info — that's where it helps you judge whether a change is yours or a surprise. When everything is affirmed the banner shows only the project's `✓` lines and no warning; a project with no instruction files of its own and all globals affirmed gets no banner at all. A trailing `ℹ` line summarizes any `@imports` deeper than four hops, which are reported but not hashed. Claude Code stops at four hops too, so today those files do not load either; the line is there so that a change to its cap shows up instead of going silent.
...
`--since <iso>` lists only the tracked files whose mtime falls after the timestamp. When none was touched it prints `0 of N` followed by the N files it tracked, so a file edited outside the scope is visibly absent rather than silently uncounted. Otherwise it lists each touched file with its status and the subjects of any commits that touched it in that window. A file with no such commits says why: `no commits in window` only when git tracks it, otherwise `untracked`, `untracked (gitignored)` or `not in a git repo` — a gitignored `CLAUDE.md` never has commits, and saying so reads as reassurance. `continuity`'s `/wrap` runs it with the session's start time, so a change you made yourself gets summarized at the end of the session that made it rather than surfacing as a trust warning at the start of the next one. A file that was touched but still hash-matches is listed with no call to action — prompting there is the alert fatigue the gate exists to prevent.

## Scope and threat model

**In scope** is what Claude Code loads at launch: the [memory docs](https://code.claude.com/docs/en/memory), checked on 2026-09-26 and measured the same day on Claude Code 2.1.283 with an `InstructionsLoaded` hook.

- In the launch directory **and every directory above it**, past the git root: `CLAUDE.md`, `.claude/CLAUDE.md`, `CLAUDE.local.md`, and every file under `.claude/rules/`. A file from a parent directory is shown at its absolute path and marked `ancestor`. `CLAUDE.local.md` needs the gate most: it is gitignored by design, so git history cannot show a change to it. Ancestor `.claude/rules/` are not in the docs; the measurement loaded them.
- `AGENTS.md` and `.claude/AGENTS.md` in the same directories, when Claude Code reads them: by default, only when none of the three `CLAUDE.md` files exists in any of those directories. affirm follows the `instructionFiles` option in `~/.claude/settings.json`; the same option in a `--settings` file or in managed settings is invisible to a hook.
- Rules are collected recursively, following symlinks to files and directories the way Claude Code's loader does. A linked file is hashed and shown at its real path, flagged `out-of-tree` when it lives elsewhere. Skipping links, as versions before 0.7.0 did, did not stop Claude Code from loading them; it only stopped anyone from watching them.
...
- Files any of those reach via Claude Code's `@import` syntax, followed four hops, which is Claude Code's own cap. Imports are resolved relative to the importing file (with `~/` and absolute paths supported) and skipped inside code spans/blocks, matching Claude Code. An import that points outside the project is still hashed, just flagged `out-of-tree`. Imports deeper than four hops are reported in the banner but not hashed.

**Out of scope:**

- `~/.claude/settings.json`, hooks, and skills. Those execute rather than instruct; hashing them is a different job.
- `CLAUDE.md` and `CLAUDE.local.md` in subdirectories *below* the launch directory. Claude Code loads them on demand, when it reads a file there, not at launch.
- The managed-policy `CLAUDE.md` (`/Library/Application Support/ClaudeCode/CLAUDE.md`, `/etc/claude-code/CLAUDE.md`). An administrator owns it; it is not yours to attest.
- Directories added with `--add-dir`. Their instruction files load only under `CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD=1`.

This is a *speed-bump* against prompt injection, not a guarantee. It catches:

- A malicious branch merging changes to CLAUDE.md.
- A dependency or scaffolding tool dropping a `CLAUDE.md`, `CLAUDE.local.md`, `AGENTS.md` or `.claude/rules/*` into your project.
