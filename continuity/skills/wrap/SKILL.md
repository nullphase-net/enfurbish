...
1. **Ask who last wrote it, then decide whether to preserve.**
   ...
   **If the file says its open list lives in another tool, that tool owns the list.** The observable is the file: a section standing where `## Open threads` would be, naming the tool and the commands that read it. Then 5.2 judges that tool's open list and records every resolution and every new item there, 5.3 builds only the prose sections — no `## Open threads`, no item copied in from the tool — and in 5.4 content in any prose section makes the file non-empty. Nothing here names a tool; the file does.
...
2. **Read it and judge per-item what this session resolved.** For each item under "Open threads" / "Start here" / "Read first" / "Don't forget":
   ...
   - **`files_changed` is git-derived, so an *untracked* gitignored path is never in it.** Ignore rules do not apply to tracked files — one listed in `.gitignore` but committed with `git add -f` still reports as modified — so the blind spot is precisely untracked-and-ignored, which is what these artifacts usually are. A project that gitignores its own `NEXT_SESSION.md`, `IDEAS.md` or scratch notes will show four rewrites of one as nothing at all; there it only looked right because `.handoff-backup/` happened to be committed, which is that repo's convention and nothing the skill guarantees. Absence from `files_changed` is not evidence an item went untouched.
